...
import {
  authAdminController, cloudinaryController, managementAdminController, tokenController,
...
export class AdminRoute extends BaseRoute {
  ...
  protected initializeRoutes(): void {
    ...
    this.router.patch('/users/:userId/status', verifyAuth(),authorizeRole(['admin']), (req: Request, res: Response, next: NextFunction) => {
      managementAdminController.updateUserStatus(req, res, next);
    });
    this.router.get('/workers', verifyAuth(),authorizeRole(['admin']), (req: Request, res: Response, next: NextFunction) => {
      managementAdminController.getAllWorkers(req, res, next);
    });
    this.router.patch('/workers/:userId/status', verifyAuth(),authorizeRole(['admin']), (req: Request, res: Response, next: NextFunction) => {
      managementAdminController.updateWorkerStatus(req, res, next);
    });
    this.router.patch('/workers/:workerId/unverified', verifyAuth(),authorizeRole(['admin']), (req:Request, res:Response, next:NextFunction) => {
      managementAdminController.verifyWorker(req, res, next);
    });
    this.router.get('/workers/unverified',  verifyAuth(),authorizeRole(['admin']),(req:Request, res:Response, next:NextFunction) => {
      managementAdminController.unVerifiedWorkers(req, res, next);
    });
    this.router.get('/services', verifyAuth(), authorizeRole(['admin']),(req:Request, res:Response, next:NextFunction) => {
    ...
    this.router.post('/services/create', verifyAuth(), authorizeRole(['admin']),(req:Request, res:Response, next:NextFunction) => {
      console.log('/services/create');
      managementAdminController.serviceRegister(req, res, next);
    });
    this.router.patch('/services/:id/status', verifyAuth(), authorizeRole(['admin']),(req: Request, res: Response, next: NextFunction) => {
