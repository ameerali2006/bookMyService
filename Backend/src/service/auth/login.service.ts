...
export class LoginService implements ILoginService{
    ...
    async execute(user: LoginDto): Promise<{success:boolean,message:string, accessToken: string|null; refreshToken: string|null; user: AdminDataDTO| UserDataDTO | responseDto|null; }> {
        try {
            let repository
        if(user.role=="admin"){
        ...
        if(user.role!='admin'){
            if((userData as IUser|IWorker).isBlocked){
                return {success:false,message:user.role=="user"?MESSAGES.USER_BLOCKED:MESSAGES.WORKER_BLOCKED, accessToken:null, refreshToken:null,user:null }
            }
        }
        ...
        return {success:true,message:MESSAGES.LOGIN_SUCCESS, accessToken, refreshToken,user:userData };
        } catch (error) {
            console.error(error)
            throw new CustomError(
                MESSAGES.INVALID_CREDENTIALS,
