...
export default function WorkerManagement() {
  ...
  const fetchData = async () => {
    try {
      ...
      setLoading(true);
      const response = await adminManagement.getUnverifiedWorkers(currentPage, pageSize);
  ...
  const handleApprove = async() => {
    ...
    try {
      ...
      const response = await adminManagement.verifyWorker(selectedWorker._id,"approved")
      if(response.data.success){
        setWorkers(prev =>
        prev.map(w =>
          w._id ? { ...w, status: response.data.status } : w
        )
        );
  ...
  const handleReject = async() => {
    if (!selectedWorker) return
    try {
      const response = await adminManagement.verifyWorker(selectedWorker._id,"rejected")
      if(response.data.success){
        setWorkers(prev =>
        prev.map(w =>
          w._id === selectedWorker._id ? { ...w, status: response.data.status } : w
        )
        );
        SuccessToast("verified successfully,")
        setOpen(false);
          ...
          {selectedWorker && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <p><span className="font-semibold">Category:</span> {selectedWorker.category.category}</p>
                <p><span className="font-semibold">Experience:</span> {selectedWorker.experience}</p>
                <p><span className="font-semibold">Status:</span> {selectedWorker.isVerified}</p>
              </div>
